...
describe('Request', () => {
    ...
    describe('#constructor', () => {
        ...
        it('does not parse a body that is not a string', () => {
            ...
            event.body = { hello: 'world' };
            request = new Request(event);
            expect(request.getBody()).toEqual(event.body);
        });
        it('can have a string body with unknown encoding', () => {
            event = apiGatewayProxyEvent.get({ headers: { 'Content-Type': ['text/html']}}) as unknown as LambdaProxyEvent;
            event.body = 'a random string';
            request = new Request(event);
            expect(request.getBody()).toEqual(event.body);
        });
        it('accepts urlEncoded data and transforms body to json object', () => {
            const body = 'email_channel=nina%40gibtsnicht.com.uat1&mobile_channel=&Onkologie=on&Immunologie=on&Psoriasis=on&Psoriasis+Arthritis=on&Diabetes=on&HIV%2FAIDS=on&Hepatitis+C=on&Prostatakarzinom=on&Mammakarzinom=on&Multiples+Myelom=on&Schizophrenie=on&Ovarialkarzinom=on&Psychiatrie=on';
            event = apiGatewayProxyEvent.get({ body, headers: { 'Content-Type': ['application/x-www-form-urlencoded'] }}) as unknown as LambdaProxyEvent;
            request = new Request(event);
        ...
        it('parses headers and multiValueHeaders', () => {
            request = new Request(event);
            // @ts-ignore
            expect(request.getHeader('accept')).toEqual([...event.multiValueHeaders.Accept].join(','), event.headers.Accept);
        });
    });
    ...
    describe('#getters', () => {
        ...
        it('#getQueryParam string', () => {
            expect(request.getQueryParam<string>('foo')).toEqual(event.queryStringParameters.foo);
        });
        it('#getQueryParam long string mistaken as float', () => {
            event.queryStringParameters.provider = '512340d314054eeda6bc0bba29a7059b';
            expect(request.getQueryParam<string>('provider')).toEqual(event.queryStringParameters.provider);
        });
        ...
        it('#getQueryParam: fetches and casts a boolean', () => {
        ...
        it('#getIdentity', () => {
            expect(request.getIdentity()).toEqual(event.requestContext.identity);
        });
        it('#getIp', () => {
        ...
        it('#getToken', () => {
            ...
            expect(request.getToken()).toEqual('token');
        });
    });

    describe('setters', () => {
        it('#setToken', () => {
            expect(request.setToken('token')).toBeUndefined();
            expect(request.getToken()).toEqual('token');
        });

        it('#addToIdentity', () => {
            request.addToIdentity({ extra: 'prop' });
            expect(request.getIdentity().extra).toEqual('prop');
        });
    })
});
